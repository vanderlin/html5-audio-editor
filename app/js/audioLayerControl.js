function audioLayerControl(elementContext)
{
    ...
    this.save = function save(id)
    {

        var blob = this.toWave().toBlob("application/octet-stream");//.toBlobUrlAsync("application/octet-stream");

        var fd = new FormData();
            fd.append('file', blob, "new.wav");
...
{
    aalc = new audioLayerControl(document.getElementsByTagName("audiolayercontrol")[0]);
